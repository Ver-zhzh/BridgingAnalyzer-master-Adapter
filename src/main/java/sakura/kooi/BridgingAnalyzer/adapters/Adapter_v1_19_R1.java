...
/**
 * Version adapter for Minecraft 1.19.2 (v1_19_R1)
 * Supports Wild Update features and modern material system
 * Uses modern Bukkit API with BlockData support
 * 
 * @author Ver_zhzh
 */
public class Adapter_v1_19_R1 implements VersionAdapter {
    ...
    public Material getMaterial(String materialName) {
        // 1.19 uses modern flattened material names with mangrove additions
        try {
    ...
    public Sound getSound(String soundName) {
        // Handle sound name changes in 1.19
        switch (soundName) {
    ...
    /**
     * Map abstract particle names to 1.19 particle enum names.
     * 1.19 maintains compatibility with previous modern versions.
     */
    private Particle getParticleFromName(String particleType) {
        try {
            switch (particleType) {
                case Particles.FIREWORK:
                    // In 1.19, FIREWORKS_SPARK is still the correct name
                    return Particle.valueOf("FIREWORKS_SPARK");
                case Particles.WITCH:
                    // 1.19 uses SPELL_WITCH
